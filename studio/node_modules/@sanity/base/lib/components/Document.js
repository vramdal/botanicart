...
function Document(props) {
  const stylesheets = props.stylesheets.map(item => _react2.default.createElement('link', {
    key: item.path,
    rel: 'stylesheet',
    href: assetUrl(props.staticPath, item)
  }));

  const subresources = props.scripts.map(item => _react2.default.createElement('link', {
    key: item.path,
    rel: 'subresource',
    href: assetUrl(props.staticPath, item)
  }));
  ...
  const favicons = props.favicons.map((item, index) => _react2.default.createElement('link', {
    key: item.path + index,
    rel: 'icon',
    href: assetUrl(props.staticPath, item)
  }));
